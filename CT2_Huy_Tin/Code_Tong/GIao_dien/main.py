...
s = serial.Serial(port='COM9',baudrate=115200, timeout=0.001)
rfid = ''
rfid_truoc = ''
...
def ChoDau():    
    ...
    print('Update cho dau, gui di ' + str(send) + ' ky tu' )  
...
def xuLi():
    ...
    ################## dung trong thuc te neu co 2 camera
    # frame_sau = xuli_camera(frame_sau_tmp)
    # vehicle, LpImg, cor = get_plate(frame_sau_tmp)
    ##################


    ################## đưa ảnh đầu vào để dễ dàng test
    path = "Plate_examples/vietnam_car_rectangle_plate.jpg"
    frame_sau = cv2.imread(path)
    ...
    if (LpImg == 0):
        ...
        binary = cv2.threshold(blur, cv2.ADAPTIVE_THRESH_MEAN_C, 255,
                            cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]# chu trang

        binary2 = cv2.threshold(binary, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 255,
                               cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]# chu trang
        ...
        # creat a copy version "test_roi" of plat_image to draw bounding box
            ...
            if(len(mycursor.fetchall()) == 0):
                ...
                imgTruoc,imgSau = TimAnh(final_string)
...
def LuuAnh(final_string,IMG_PATH_1,IMG_PATH_2):
    ...
    cv2.imwrite('Anh_chup/'+ final_string +'_'+ date_now + time +'_'+'.png', img)
    filePath = 'Anh_chup/'+ final_string +'_'+ date_now + time +'_'+'.png'
